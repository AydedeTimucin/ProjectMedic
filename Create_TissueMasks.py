...
def create_mask(args:list, wanted_rlength, create_rgb_image=False, additonal_info=False):
    ...
    # Get the resolution of the slide
    try:
        ...
        try:
            ...
            raise Exception('Unknown Val_x for: ' + slide_path)
    ...
    # Get the area ratio of the mask
...
mask_info_df = pd.DataFrame(columns=["slide_id", "source", "slide_path", "wanted_rlength", "roi_str"])
additional_info_df = pd.DataFrame(columns=["slide_id","source", "Mask_area_px", "im_area_px", "Mask_area_ratio"])
